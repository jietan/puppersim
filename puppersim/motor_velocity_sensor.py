...
@gin.configurable
class MotorVelocitySensor(sensor.Sensor):
  ...
  def set_robot(self, robot):
    ...
    if self._observe_sine_cosine:
      ...
      upper_bound = _convert_to_np_array(
          self._robot.motor_limits.velocity_upper_limits, self._robot.num_motors)
  ...
  def _get_original_observation(self):
    if self._observe_sine_cosine:
      return self._robot.timestamp, np.hstack(
          (np.cos(self._robot.motor_velocities), np.sin(self._robot.motor_velocities)))
    else:
      return self._robot.timestamp, self._robot.motor_velocities
